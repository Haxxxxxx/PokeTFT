...
export function FriendsPanel() {
  ...
  return (
    <div className="panel w-full flex flex-col gap-3 rounded-xl p-4">
      ...
      {isGuest ? (
        <p className="text-xs text-slate-500">{t.fr_create_acct}</p>
      ) : (
        <>
          <div className="flex gap-2">
            ...
            />
            <button onClick={add} className="px-3 py-2 rounded-lg bg-sky-700 hover:bg-sky-600 text-xs font-bold text-white">{t.fr_add}</button>
          </div>
          {msg && <p className="text-[11px] text-slate-400">{msg}</p>}

          <div className="flex flex-col gap-1.5 max-h-[260px] overflow-y-auto">
            {friends.length === 0 && <p className="text-xs text-slate-600">{t.fr_empty}</p>}
            {friends.map((f) => (
              <div key={f.uid} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-slate-800/50 group">
                <button onClick={() => openUserProfile(f.uid)} title={t.pt_view} className="flex items-center gap-2.5 flex-1 min-w-0 text-left">
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className="text-[10px] text-slate-500">{f.currentGame ? `${t.fr_in_game} ${f.currentGame}` : f.online ? t.fr_online : t.fr_offline}</span>
                  </span>
                </button>
