...
export function SignInScreen() {
  ...
  const inputCls = "w-full bg-slate-950/50 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-amber-500/80 focus:ring-2 focus:ring-amber-500/20 transition-all";
  ...
  return (
    <AuthShell subtitle="Sign in to play with friends">
      <button
        ...
        disabled={busy}
        className="flex items-center justify-center gap-3 w-full py-3 rounded-xl font-bold text-sm bg-white text-slate-800 hover:bg-slate-100 hover:shadow-lg hover:shadow-white/10 disabled:opacity-50 transition-all"
      >
      ...
      <button onClick={signInGuest} disabled={busy} className="text-xs text-slate-500 hover:text-amber-300 transition-colors -mt-1">
        or play as guest →
      </button>
    </AuthShell>
  );
