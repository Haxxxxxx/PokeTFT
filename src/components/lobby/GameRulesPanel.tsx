...
/** Gilded section header — a gold-tinted label with a hairline divider, so each
 *  group reads as its own panel section instead of a flat list. */
...
const chipBase =
  "rounded-lg border text-xs font-bold transition-all disabled:cursor-not-allowed";
const chipActive =
  "bg-amber-950/40 border-amber-500/70 text-amber-200 shadow-[0_0_16px_-6px_rgba(212,175,55,0.85)]";
const chipIdle =
  "bg-slate-900/45 border-slate-700/70 text-slate-400 hover:border-amber-700/50 hover:text-slate-200";
...
export function GameRulesPanel({ isHost }: { isHost: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Generations */}
      <section>
        <SectionHeader title={t.r_gens} badge={t.r_pool(poolCount)} />
      ...
      {/* Items */}
      <section>
        <SectionHeader title={t.r_items} badge={`${activeItems}/${COMPLETED.length}`} />
        <div className="grid grid-cols-2 gap-1.5">
          {COMPLETED.map((item) => {
