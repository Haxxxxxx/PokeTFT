...
type Slide = { icon: string; title: { en: string; fr: string }; body: { en: string; fr: string } };
...
export function HowToPlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-[480px] rounded-2xl border border-slate-700 bg-slate-900 p-7 flex flex-col gap-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">{lang === "fr" ? "Comment jouer" : "How to play"} · {i + 1}/{SLIDES.length}</span>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300 text-xl leading-none">×</button>
        </div>

        <div className="flex flex-col items-center text-center gap-3 py-2 min-h-[180px] justify-center">
        ...
        <div className="flex items-center gap-1.5 justify-center">
          {SLIDES.map((_, k) => (
            <button key={k} onClick={() => setI(k)} className={`h-1.5 rounded-full transition-all ${k === i ? "w-5 bg-amber-400" : "w-1.5 bg-slate-700 hover:bg-slate-600"}`} />
          ))}
        </div>
