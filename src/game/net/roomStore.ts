  ...
  host: async (name, rules) => {
    ...
    try {
      ...
      await set(roomRef(code), {
        ...
        rules: { startingHp, maxPlayers, generations: rules?.generations ?? [1], itemsEnabled: rules?.itemsEnabled ?? [], draftPoolSize: rules?.draftPoolSize ?? 60, augmentsEnabled: rules?.augmentsEnabled !== false, serverDriven: rules?.serverDriven !== false },
